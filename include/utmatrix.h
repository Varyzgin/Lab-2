...
bool TVector<ValType>::operator!=(const TVector& v) const {
	...
	for (int i = 0; i < Size; i++)
		if (v.pVector[i] != pVector[i])
			return true;
	return false;
	//или просто return !(*this == v);  (отрицание ==)
}
...
TVector<ValType> TVector<ValType>::operator+(const TVector<ValType>& v) {
	if (v.Size != Size || v.StartIndex != StartIndex)
		throw("Vectors are have different dimmention");
	TVector<ValType> res(Size);
	for (int i = 0; i < Size; i++)
...
TVector<ValType> TVector<ValType>::operator-(const TVector<ValType>& v) {
	if (v.Size != Size || v.StartIndex != StartIndex)
		throw("Vectors are heve different dimmention");
	TVector<ValType> res(Size);
	for (int i = 0; i < Size; i++)
		res.pVector[i] = v.pVector[i] - pVector[i];
	return res;
